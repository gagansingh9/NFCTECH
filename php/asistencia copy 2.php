...
$dia_semana = date('w');
$lunes_llegada =    date("H:i:s");  
$lunes_limite = date('H:i:s', strtotime('3:15PM'));
$lunes_retraso = date('H:i:s', strtotime('3:25PM'));
$lunes = date('H:i:s', strtotime('3:25:01PM'));
// Obtener el día de la semana (0 = domingo, 6 = sábado)
if ($dia_semana == 1 ) {
while (strtotime($lunes_llegada) <= $lunes_limite)  {
  $estado = 'asistencia';
  $save = mysqli_query($conn, "INSERT INTO registro (id, horallegada, horalimite) VALUES ('$id_usuario', '$lunes_llegada', '$lunes_limite') ON DUPLICATE KEY UPDATE id='$id_usuario', horallegada='$lunes_llegada', horalimite = '$lunes_limite' ");

  $save2 = mysqli_query($conn,"UPDATE registro SET estado = '$estado'  WHERE id = $id_usuario ");


 }  if (strtotime($lunes_retraso) ){
